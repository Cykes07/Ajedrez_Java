...
public class App extends Application {
    ...
    public void start(Stage stage) {
        ...
        Label t0 = new Label("¡ HORA DE JUGAR !");
        t0.setStyle("-fx-font-size: 20px; -fx-text-fill: red");
        t0.setPadding(new Insets(0, 0, 40, 0));
        Label t1 = new Label("Turno de: ");
        t2.setText("Fichas Blancas");
        ...
        vb.getChildren().addAll(t0,t1, t2);
        ...
        boton.setOnMouseClicked(event -> {
            if (event.getButton() == MouseButton.PRIMARY) {
                if (selectedButton == null) {
                    ...
                    if(selectedButton.getId().equals("blanco")){
                        for (int i = 0; i < 8; i++) {
                            for (int j = 0; j < 8; j++) {
                                if(columnaDestino==0){
                                    ImageView selectedImageView = (ImageView) selectedButton.getGraphic();
                                    selectedImageView.setImage(new Image("Fichas/reinaB.png"));
                                    Reina r= new Reina("Fichas/reinaB.png",true);
                                    tablero[columnaOrigen][filaOrigen]=r;
                                }
                            }
                        }
                    }
                    
                    else if(selectedButton.getId().equals("negro")){
                        for (int i = 0; i < 8; i++) {
                            for (int j = 0; j < 8; j++) {
                                if(columnaDestino==7){
                                    ImageView selectedImageView = (ImageView) selectedButton.getGraphic();
                                    selectedImageView.setImage(new Image("Fichas/reinaN.png"));
                                    Reina r= new Reina("Fichas/reinaN.png",false);
                                    tablero[columnaOrigen][filaOrigen]=r;
                                }
                            }
                        }
                    ...
                    if (piezaOrigen != null && piezaOrigen.esMovimientoValido(filaOrigen, columnaOrigen, filaDestino, columnaDestino)) {
                        ...
                        if (boton.getId() != null && boton.getId().startsWith(turnoBlanco ? "negro" : "blanco")) {
                            ...
                            if(turnoBlanco==false){
                                    t2.setText("Fichas Negras");
                            }
                            else{
                                    t2.setText("Fichas Blancas");
                            }
    ...
    public static String obtenerIdDeBoton(int fila, int columna) {
    ...
    return null; // Retorna null si no se encuentra ningún botón en la fila y columna especificadas
}
